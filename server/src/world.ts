...
function elevation(worldX: number, worldY: number): number {
  return valueNoise(worldX * 0.035, worldY * 0.035, WORLD_SEED) * 0.58
    + valueNoise(worldX * 0.09, worldY * 0.09, WORLD_SEED + 17) * 0.3
    + valueNoise(worldX * 0.2, worldY * 0.2, WORLD_SEED + 31) * 0.12;
}
...
export function generateChunk(chunkX: number, chunkY: number): WorldChunk {
  ...
  for (let localY = 0; localY < CHUNK_SIZE; localY += 1) {
    for (let localX = 0; localX < CHUNK_SIZE; localX += 1) {
      ...
      const kind = tileAtWorld(worldX, worldY);
      tiles[localY * CHUNK_SIZE + localX] = kind;
      const roll = hash(worldX * 3 + 11, worldY * 5 + 7, WORLD_SEED + 101);
      if (kind === TileKind.Grass && roll > 0.985) resources.push({ id: nodeId("wood", worldX, worldY), type: "wood", x: worldX, y: worldY });
      else if (kind === TileKind.Rock && roll > 0.96) resources.push({ id: nodeId("stone", worldX, worldY), type: "stone", x: worldX, y: worldY });
